...
NUM_LOSSES_TO_SAVE = 5


class FedPIDAvgClient(fl.client.NumPyClient):
    """
    The flower client class for Federated PID Weighted Averaging. Keeps track of the last few losses.
    ...
    def fit(self, parameters, config: Dict[str, Scalar]):
        ...
        Compute loss differential and integral (equation 5 and 6 in https://arxiv.org/pdf/2304.12117.pdf)
        ...
        Send the updated model, number of examples used for training, loss differential and loss integral to the server
        """
        ...
        if len(loss_collection) > 1:
            loss_differential = loss_collection[-2] - loss_collection[-1]

        # compute loss integral
        loss_integral = sum(loss_collection)

        return self.get_parameters(config={}), len(self.trainloader), {'loss_differential': loss_differential,
                                                                       'loss_integral': loss_integral}
...
def get_fedpidavgclient_fn(model: torch.nn.Module,
                           trainloaders: List[DataLoader],
                           valloaders: List[DataLoader]):
    """
    Helper function to create the function that generates FedPIDAvgClients
    """

    def client_fn(cid) -> FedPIDAvgClient:
        net = model.to(DEVICE)
        trainloader = trainloaders[int(cid)]
        valloader = valloaders[int(cid)]
        client_dir = Path(os.path.join(os.getcwd(), 'client_dirs/' + str(cid)))

        return FedPIDAvgClient(cid, net, trainloader, valloader, client_dir)
